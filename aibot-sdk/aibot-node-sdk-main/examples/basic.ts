...
import { generateReqId, ChatType } from '../src';
...
// 监听连接断开事件（当有新连接建立时，旧连接会收到此事件）
wsClient.on('event.disconnected_event', (frame: WsFrame) => {
  console.log('🔌 收到连接断开事件: 检测到新连接建立，当前连接将被关闭');
});

// 示例：主动发送消息（使用 chat_type 明确指定会话类型）
// 注意：需要用户先给机器人发消息，且受频率限制（30条/分钟，1000条/小时）
async function sendProactiveMessage() {
  // 发送 Markdown 消息到群聊
  // await wsClient.sendMessage('CHATID', {
  //   msgtype: 'markdown',
  //   markdown: { content: '这是一条**主动推送**的群消息' },
  //   chat_type: ChatType.Group,  // 明确指定为群聊
  // });
  ...
  //   chat_type: ChatType.Single,  // 明确指定为单聊
  // });
}

// 示例：流式消息回复（注意：需在 6 分钟内完成）
async function streamReplyWithTimeoutCheck(frame: WsFrame) {
  const streamId = generateReqId('stream');
  const startTime = Date.now();
  const SIX_MINUTES = 6 * 60 * 1000; // 6 分钟（毫秒）

  // 发送流式中间内容
  await wsClient.replyStream(frame, streamId, '正在思考中...', false);

  // 检查是否超时
  if (Date.now() - startTime > SIX_MINUTES) {
    console.warn('⚠️ 流式消息已超时（6分钟），无法再更新');
    return;
  }

  // 发送最终结果
  await wsClient.replyStream(frame, streamId, '这是最终结果', true);
  console.log('✅ 流式回复完成');
}
