...
| `sendMessage(chatid, body)` | 主动发送消息（支持 Markdown 或模板卡片），无需依赖回调帧。注意：需要用户先给机器人发消息，且受频率限制（30条/分钟，1000条/小时） | `Promise<WsFrame>` |
...
wsClient.replyStream(
  ...
  msgItem?: ReplyMsgItem[], // 图文混排项（仅 finish=true 时有效）。注意：官方标注此字段暂不支持
  feedback?: ReplyFeedback, // 反馈信息（仅首次回复时设置）
);
```

**重要限制**：从流式消息发送开始，需在 **6 分钟内** 完成所有刷新并设置 `finish=true`，否则消息将自动结束。
...
await wsClient.sendMessage('chatid', {
  ...
  chat_type: ChatType.Group,  // 明确指定会话类型
});
...
await wsClient.sendMessage('userid', {
  ...
  chat_type: ChatType.Single,  // 明确指定会话类型
});
```
...
- **会话类型**：建议通过 `chat_type` 明确指定单聊（`ChatType.Single`）或群聊（`ChatType.Group`），避免混淆
...
| `event.disconnected_event` | `frame: WsFrame<EventMessage>` | 收到连接断开事件（当有新连接建立时，旧连接会收到此事件） |
...
| --- | --- | --- |
| `Text` | `'text'` | 文本消息 |
| `Image` | `'image'` | 图片消息（URL 已加密，使用消息中的 `image.aeskey` 解密）。**仅支持单聊** |
| `Mixed` | `'mixed'` | 图文混排消息（包含 text / image 子项） |
| `Voice` | `'voice'` | 语音消息（已转文本）。**仅支持单聊** |
| `File` | `'file'` | 文件消息（URL 已加密，使用消息中的 `file.aeskey` 解密）。**仅支持单聊** |
...
| `DisconnectedEvent` | `'disconnected_event'` | 连接断开事件：当有新连接建立时，系统会给旧连接发送该事件 |
