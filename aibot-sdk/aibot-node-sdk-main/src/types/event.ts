...
export interface DisconnectedEventData {
  /** 事件类型 */
  eventtype: EventType.DisconnectedEvent;
}
...
export interface WSClientEventMap {
  ...
  /** 收到连接断开事件，body 为 EventMessage（event 字段为 DisconnectedEventData）
   * 当有新连接建立时，系统会给旧连接发送该事件并且主动断开旧连接
   */
