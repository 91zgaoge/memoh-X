...
import { WsCmd, ChatType } from './types';
...
export class WSClient extends EventEmitter<WSClientEventMap> {
   ...
   * 发送流式文本回复（便捷方法）
   *
   * 注意：从流式消息发送开始，需在 6 分钟内完成所有刷新并设置 finish=true，否则消息将自动结束。
   *
   * @param frame - 收到的原始 WebSocket 帧，透传 headers.req_id
   * @param streamId - 流式消息 ID
   * @param content - 回复内容（支持 Markdown）
   * @param finish - 是否结束流式消息，默认 false
   * @param msgItem - 图文混排项（仅在 finish=true 时有效），用于在结束时附带图片内容。注意：官方文档标注此字段暂不支持
   ...
   * @param finish - 是否结束流式消息，默认 false
   * @param options - 可选项
   * @param options.msgItem - 图文混排项（仅在 finish=true 时有效）。注意：官方文档标注此字段暂不支持
   ...
   *
   * 注意：
   * - 需要用户在会话中先给机器人发消息，后续机器人才能主动推送消息给对应会话
