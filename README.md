...
> 🏢 **企业微信**：WebSocket 实时连接，支持"思考中..."即时回复、流式消息输出、智能频率限制（30条/分钟）。

> 🐧 **QQ 频道**：基于官方 Bot API，支持 QQ 频道的消息收发和事件处理。
...
本项目在 [Memoh](https://github.com/memohai/Memoh) 基础上进行了大量二次开发，新增以下功能：

### 🏢 企业微信适配器（全新开发）

- **WebSocket 实时连接**：基于企业微信 WebSocket 接口，实现低延迟消息收发
- **"思考中..."即时回复**：收到消息后立即返回"思考中..."提示，提升用户体验
- **流式消息输出**：支持流式响应，实时显示 AI 生成内容
- **智能频率限制**：内置 30条/分钟、1000条/小时的频率限制，避免触发企业微信限流
- **完整的消息类型支持**：支持文本、图片、Markdown、卡片、文件等多种消息格式
- **群聊文件发送**：支持在群聊中发送生成的文件（MD、Excel、Word 等）
- **@提及功能**：群聊中自动 @提及请求者，提升交互体验

### 🐧 QQ 频道适配器

- **官方 Bot API 支持**：基于 QQ 官方 Bot 接口开发
- **WebSocket 事件接收**：实时接收频道消息、成员变动等事件
- **丰富的交互能力**：支持消息发送、表情回应、成员管理等操作
- **完善的错误处理**：包含重连机制、心跳保活、异常恢复

### 🔧 系统稳定性增强

- **DNS 解析修复**：修复容器环境下 DNS 配置失效问题，使用 systemd-resolved 上游 DNS
- **Discord 适配器修复**：修复图片消息处理、流式输出等问题
- **模型列表增量渲染**：优化模型列表加载性能，支持增量渲染
...
- **MCP OAuth 2.0**：支持需要 Bearer Token 认证的 MCP 服务器
- **Workspace 架构**：全新容器管理架构，支持快照版本管理
- **WeCom 图片识别修复**：修复图片消息无法识别的问题

### 2026-03-19 消息隔离 + Kimi Code 支持
- **WeCom 消息隔离**：修复单聊用户间消息串扰问题
- **Kimi Code 支持**：添加 Kimi Code 模型支持（多模态图片识别）
- **思考动画优化**：添加波浪动画效果
