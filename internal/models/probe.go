...
func (s *Service) Test(ctx context.Context, id string) (TestResponse, error) {
	...
	if model.Type == string(ModelTypeEmbedding) {
		...
		result = probeChatModel(ctx, baseURL, apiKey, model.ModelID, clientType)
	}
...
func probeChatModel(ctx context.Context, baseURL, apiKey, modelID string, clientType string) probeResult {
	switch clientType {
	case "openai":
